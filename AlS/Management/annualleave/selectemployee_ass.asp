...
'---------------------------------
		...
		strQuery = "select a.StaffID, b.FirstName + ' ' + isnull(b.MiddleName, '') + ' ' + b.LastName as FullName, a.DirectLeaderID, " &_
				...
				"AND a.StaffID not in (Select y.StaffID From ATC_EmployeeAnnualLeave y INNER JOIN " & _
										"(SELECT StaffID, MAX(Applydate) as lastdate FROM ATC_EmployeeAnnualLeave GROUP BY StaffID) w " & _
										"ON y.StaffID=w.StaffID AND Applydate=lastdate " & _
...
if session("READYUSER") then
	...
	if strAct="SAVE" then
		...
		dateApply=cdate(Request.Form("lstMonthF") & "/" & Request.Form("lstDayF") & "/" & Request.Form("lstYearF"))
		...
		if ret then
			...
			Do Until rsUser.EOF
				if arrTmp(0, i) <> 0 then		  
					strQuery = "INSERT INTO ATC_EmployeeAnnualLeave (StaffID ,RuleID,Applydate) VALUES (" & _
								 rsUser("StaffID") & "," & intRuleID & ",'" & dateApply & "')"						 
				...
				rsUser.MoveNext
			Loop
'Response.Write (gMessage="") & "--" & strQuery
			set arrTmp = nothing
			if gMessage<>"" then 
...
function CheckData(){
	var dateFrom=document.selectemployee.lstdayF.value + "/" + document.selectemployee.lstmonthF.value + "/" + document.selectemployee.lstyearf.value
        ...
        <table width="100%" border="0" cellpadding="0" cellspacing="0">
          <tr bgcolor=<%if gMessage="" then%>"FFFFFF"<%else%>"#E7EBF5"<%end if%>>
			...
			<tr> 
			  ...
			  <td class="blue-normal" width="22%" valign="middle" align="right"> Search for&nbsp; &nbsp; &nbsp; &nbsp; </td>
          ...
          <tr> 
            <td class="title" height="50" align="center" colspan="4"> Select Employees For Annual Leave Rule</td>
          </tr>
          <tr> 
            <td class="blue" height="30" align="center" colspan="4">Apply from:
