...
<%
	...
	if isnull(rsANUser) then
		...
		rsANUser.MoveFirst
'response.write 	rsANUser.recordcount
On Error resume next
		if rsANUser.recordcount>0 OR not rsANUser.EOF then
...
function back_menu()
{
	window.document.frmtms.action = "tms_list_staff.asp";
	window.document.frmtms.target = "_self";
	window.document.frmtms.submit();
}
...
<%Response.Write strHistoryRate %>							      
			  ...
			  <tr> 
           			<td bgcolor="#FFFFFF" class="blue-normal">&nbsp;* Balance to use before <b><%=day(expiredDateThisYear) & "-" & MonthName(month(expiredDateThisYear),true) & "-" & year(expiredDateThisYear)%></b>:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
           				<span class="red"><b>
					<%if cdbl(dblBalanceLastYear)>cdbl(dblApplicationReserveBeforeExpired) then
'response.write 		dblBalanceLastYear & "--" & dblApplicationReserveBeforeExpired		& "--" &  	cdbl(dblBalanceLastYear)-cdbl(dblApplicationReserveBeforeExpired)
					%> 
					
           					<%=formatnumber((cdbl(dblBalanceLastYear)-cdbl(dblApplicationReserveBeforeExpired))/cdbl(dblWorkingHour),2)%> <%else%>0.00<%end if%>&nbsp; (days)</b></span>
           			</td>
			  </tr>	
					<%end if
			  end if%>			  
