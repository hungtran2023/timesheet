...
function sort(kind)
{
	document.frmreport.action = "tms_select_staff.asp?act=vps" + kind + "&view=" + "<%=strView%>";
...
function viewpage(kind)
{
	...
	if (kind == 1)
	{
		...
		{
			document.frmreport.action = "tms_select_staff.asp?act=vpa" + kind + "&view=" + "<%=strView%>";
			document.frmreport.submit();
		}	
	}
	else
...
function search(kind)
{
	if (kind == "1")
	{
		...
		{
			document.frmreport.action = "tms_select_staff.asp?act=vra" + kind + "&view=" + "<%=strView%>";
			document.frmreport.submit();	
		}
	}	
	else
...
var view = "<%=strView%>";
	if (view == "t")
		...
		if(view == "d")
			opener.document.frmtms.action = "tms_viewdetails.asp?act=vmya";	
		else 
			if (view == "a")
				opener.document.frmtms.action = "tms_approval.asp";
			else
				opener.document.frmtms.action = "staff_view_leave.asp";
    ...
    <tr bgcolor="#FFFFFF">
      <td align="center"> 
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          ...
          <tr>
            <td bgcolor="#FFFFFF" height="20" class="blue-normal">&nbsp;&nbsp;* Click on each column header to sort the list by alphabetical order. 
            </td>
          </tr>
        </table>
      </td>
