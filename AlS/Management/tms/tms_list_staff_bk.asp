	...
	If Not rsSrc.EOF Then
		For i = 1 To psize
			...
			strOut = strOut & "<tr bgcolor=" & strColor & ">" &_
			         ...
			         "<td valign='top'  class='blue-normal'>" & Showlabel(rsSrc("Department")) & "</td>"
			if fgHREnter then					 
			    strOut = strOut &  "<td valign='top'  class='blue-normal' align='center'><a href='" & MakeHTTP_() & "aisnet/HREnterTimeSheet/HREnterTimeSheet/" & rsSrc("StaffID") & "' " &_
			         "class='c'>--</a></td>"
			end if
...
function sort(kind)
{
	document.frmreport.action = "tms_list_staff.asp?act=vps" + kind;
...
function viewpage(kind)
{
	...
	if (kind == 1)
	{
		...
		{
			document.frmreport.action = "tms_list_staff.asp?act=vpa" + kind;
			document.frmreport.submit();
		}
	}
	else
...
function search(kind)
{
	if (kind == "1")
	{
		...
		{
			document.frmreport.action = "tms_list_staff.asp?act=vra" + kind;
			document.frmreport.submit();
		}
	}
	else
...
function viewtms(varid)
{
	...
	window.document.frmreport.action = "timesheet.asp";
...
%>
			    ...
			    <tr align="center">
				  <td class="title" height="50" align="center" colspan="5">Write Timesheet For Employees</td>
			    </tr>
			  </table>
		    </td>
		  </tr>
		                ...
		                <td bgcolor="#617DC0">
		                  <table width="100%" border="0" cellspacing="1" cellpadding="5">
		                    <tr bgcolor="#8CA0D1">
		                      ...
		                      <td class="blue" align="center" ><a href="javascript:sort('d')" onMouseOver="self.status='Sort by Department';return true" onMouseOut="self.status='';return true" class="c">Department</a></td>
<%if fgHREnter then	%>							  
								<td class="blue" align="center" width="10%">Timesheet for Duration</td>
<%end if%>								
		                    </tr>
<%
		                  ...
		                  <table width="100%" border="0" cellspacing="0" cellpadding="0">
		                    <tr>
		                      <td bgcolor="#FFFFFF" height="20" class="blue-normal" width="76%">&nbsp;&nbsp;* Click
		                        on the exact name to write timesheet.</td>
