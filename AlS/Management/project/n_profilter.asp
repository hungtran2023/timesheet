...
'--------------------------------------------------
	...
	if varAct = "APPLY" then		
		...
		if criteria <>"" then filter_criteria = filter_criteria & " AND ProjectTypeID =" & criteria
		criteria = Request.Form("chkinhouse")
		if criteria <> "" then 
			filter_criteria = filter_criteria & " AND ProjectKey2=1"
		else
			filter_criteria = filter_criteria & " AND (ProjectKey2=5 OR ProjectKey2=7)"
		end if
...
<!--
	...
	window.opener.document.forms[0].action = scriptname + "?act=vra3";
...
<script>
function apply() {
	document.filter.action = "n_profilter.asp?act=APPLY";
	document.filter.target = "_self";
	document.filter.submit();
}

function clear() {
	document.filter.action = "n_profilter.asp?act=CLEAR";
	document.filter.target = "_self";
	document.filter.submit();
}
</script>
...
<%Response.Write strOut%>
          ...
          <tr bgcolor="C0CAE6"> 
            <td width="99" class="blue-normal" align="right" height="26">Billable 
              Project &nbsp; </td>
