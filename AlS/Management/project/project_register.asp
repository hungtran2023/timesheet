...
dim blnisBIM,dblContingency, blnDesigned, dblBDM, dblCSOGrossProfit
Dim intTrackerID,intStatus,dblProposal,dblAwarded
'--------------------------------------------------------------------------------
'Get Project Tracking
'--------------------------------------------------------------------------------
sub GetTrackingInformation(strProjectID)
	dim strSql,rsSrc
	
	strSql="SELECT * FROM ATC_ProjectTracking WHERE ProjectID = '"&strProjectID&_
		"' AND DateTracking = (SELECT MAX(DateTracking) FROM [dbo].[ATC_ProjectTracking] WHERE ProjectID = '"&strProjectID&"')"
	
	Call GetRecordset(strSql,rsSrc)
	
	if (rsSrc.RecordCount>0) then	
		intTrackerID=rsSrc("ProjTrackerID")
		intStatus=rsSrc("ProjStatus")
		dblProposal=rsSrc("ProposalValue")
		dblAwarded=rsSrc("AwardedValue")
	else
		intTrackerID=-1
		intStatus=1
		dblProposal=0
...
'--------------------------------------------------------------------------------
function InsertProjectTracking(strProjectID)
	
	
	intTrackerID=Request.Form("txtTrackerID")
	if intTrackerID="" then intTrackerID=-1
		
	intStatus=Request.Form("lbStatus")
	if intStatus="" then intStatus=1
	
	if Request.Form("txtProposalValue")="" then 
		dblProposal=0
	else
		dblProposal=cdbl(Request.Form("txtProposalValue"))
	end if
	if Request.Form("txtAwardedValue")="" then 
		dblAwarded=0
	else
		dblAwarded=cdbl(Request.Form("txtAwardedValue"))
	end if
	...
	If objDatabase.dbConnect(strCnn) Then	
			...
			myCmd.CommandText = "InsertProjectTracking"		

			Set myParam = myCmd.CreateParameter("ProjTrackerID",adInteger,adParamInput)
			myCmd.Parameters.Append myParam
			Set myParam = myCmd.CreateParameter("ProjectID", adVarChar,adParamInput,15)
			myCmd.Parameters.Append myParam	
			Set myParam = myCmd.CreateParameter("ProjStatus", adInteger,adParamInput)
			myCmd.Parameters.Append myParam					
		
			Set myParam = myCmd.CreateParameter("ProposalValue",adDecimal,adParamInput,20)
			myParam.Precision=18
			myParam.NumericScale=2	
			myCmd.Parameters.Append myParam
			
			Set myParam = myCmd.CreateParameter("AwardedValue",adDecimal,adParamInput,20)
			myParam.Precision=18
			myParam.NumericScale=2	
			myCmd.Parameters.Append myParam
			...
			myCmd("ProjTrackerID")	= intTrackerID			
			myCmd("ProjectID")		= strProjectID
			myCmd("ProjStatus")		= intStatus			
			myCmd("ProposalValue")	= dblProposal			
			myCmd("AwardedValue")	= dblAwarded			
			myCmd("StaffID")		= intUserID

'response.write intTrackerID & "##" & strProjectID & "##" & intStatus& "##" &dblProposal& "##" &dblAwarded& "##" &intUserID
'response.end
	...
	InsertProjectTracking=strError
...
if strAction="" then
	...
	if not rsProject.EOF then
		...
		dblCSOGrossProfit=rsProject("CSOGrossProfit")
		
		GetTrackingInformation(strFullProjectID)
	...
	Select Case strAction
		...
		case "save"
			...
			elseif fgStatus="New" then
				...
				if strError="" then
					Call InsertProjectTracking(strProjectIDkey(0) & strProjectIDkey(1) & strProjectIDkey(2) & strProjectIDkey(3)& strProjectIDkey(4)& strProjectIDkey(5)& strProjectIDkey(6))
				end if
			'For Edit project
			else
				If fgStatus = "Issued" Then					
					...
					dblCSOGrossProfit=Request.Form("txtGrossprofit")
					if dblCSOGrossProfit="" then dblCSOGrossProfit=null
...
'Response.end						
				Call UpdateProject(strFullProjectID,strProjectIDkey,strDateTranfer,intDepartID,strProjectkey2,strProjectName,_
						...
						strClientCountry,strCurrency,strCSOFilename,blnisBIM,blnDesigned,dblContingency,dblBDM,dblCSOGrossProfit,strError)
				if strError="" then
					Call InsertProjectTracking(strFullProjectID)	
				end if
			end if
...
<%If fgStatus <> "" and fgStatus <> "New" then%>
					...
					<tr> 
						<td>&nbsp;<input name="txtTrackerID" type="hidden"  value="<%=intTrackerID%>"></td>
                        <td colspan="2" class="blue-normal"><b><u>Project Status</u></b>
						</td>
                    </tr>
...
%>				
                        <td>&nbsp;</td>
                        <td class="blue-normal">Status</td>
                        <td class="blue-normal">
							<select class='blue-normal' name='lbStatus' style='WIDTH: 72px; HEIGHT: 20px'>
								<%for ii=0 to UBound(arrStatus)%>
									<option value="<%=(ii+1)%>" <%If intStatus=ii+1 then%>selected<%end if%>><%=arrStatus(ii)%></option>
					   ...
					   <tr> 
                        <td>&nbsp;</td>
                        <td class="blue-normal">Proposal Value</td>
                        <td class="blue-normal">
                          <input name="txtProposalValue" size="10" style="WIDTH: 72px; HEIGHT: 20px" class="blue-normal" value="<%=IIf(cdbl(dblProposal)=0, "", dblProposal )%>">&nbsp;			
		...
		loop%>
                       ...
                       <tr> 
                        <td>&nbsp;</td>
                        <td class="blue-normal">Awarded Value</td>
                        <td class="blue-normal">
                          <input name="txtAwardedValue" size="10" style="WIDTH: 72px; HEIGHT: 20px" class="blue-normal" value="<%=IIf(cdbl(dblAwarded)=0, "", dblAwarded)%>"/>&nbsp;&nbsp;&nbsp; 
                         </td>
                      </tr>
		...
		<%If strProjectIDkey(3)<>"T" THEN%>
                          ...
                          <input name="txtCost" type="hidden" size="10" style="WIDTH: 72px; HEIGHT: 20px" class="blue-normal" value="<%=dblValue%>">&nbsp;
		<%elseif strProjectIDkey(3)="T" then%>                    

                          <input name="txtDailyRate" type="hidden" size="10" style="WIDTH: 72px; HEIGHT: 20px" class="blue-normal" value="<%=dblDailyRate%>">&nbsp;
		<%end if%>
                      <tr> 
                        <td>&nbsp;</td>
                      ...
                      <tr> 
                        <td>&nbsp;</td>
                        <td class="blue-normal">Gross profit</td>
                        <td class="blue-normal">
                          <input name="txtGrossprofit" size="10" style="WIDTH: 72px; HEIGHT: 20px" class="blue-normal" value="<%=dblCSOGrossProfit%>"/>&nbsp;&nbsp;&nbsp; 
                         </td>
                      </tr>
...
<%	else%>
                        ...
                        <td class="blue-normal">
						  ...
						  <input name="txtGrossprofit" type="hidden" value="<%=dblCSOGrossProfit%>"/>						  
                        </td>
                      </tr>
                      <tr>
						<td>&nbsp;</td>
