...
Sub CopyDataPrivate(ByRef rsSrc, ByRef rsDes)
	...
	Do While not rsSrc.EOF
	    rsDes.AddNew Array("pID", "pName", "sID", "sName", "sParentID", "chainID", "fgActivate"), _
...
Sub SetAttRsPrivate(ByRef rsSrc)
  ...
  ' Append some Fields to the Fields Collection
	...
	rsSrc.Fields.Append "fgActivate", adTinyInt
...
Dim strTmp, i
	...
	Else
		strOut = Showlabel(strpID) & " - " & strOut
	End If
...
'-----------------------------------
'Check ACCESS right. Because this page and 'listofemp.asp' page is one
'-----------------------------------
	strFilename = Request.Form("txtpreviouspage")
	...
	if isEmpty(session("Righton")) then
		...
		for ii = 0 to Ubound(getRight, 2)
			if getRight(0, ii) = strFilename then
				fgRight=true
				exit for
			end if
		next
...
'------------------------------
	...
	'get fullname and jobtitle of selected person
	personID = Request.Form("txthidden")
	objEmployee.SetFullName(personID)
	personInfo = split(objEmployee.GetFullName,";")
...
if fgExecute and isEmpty(session("arrBookmark")) then
	...
	If objDb.dbConnect(strConnect) then
	  strQuery = "select distinct b.projectID, c.ProjectName, b.SubTaskID, b.SubTaskName, ISNULL(b.taskID, 0) as ParentID, ISNULL(b.ChainID, '') as ChainID, c.fgActivate FROM " &_
					"((select * from ATC_Assignments where StaffID = " & personID & ") a " &_
	  ...
	  If objDb.runQuery(strQuery) Then
	    If not objDb.noRecord Then
			...
			Do While not objDb.rsElement.EOF
			  objData.AddNew Array("pID", "pName", "sID", "sName", "sParentID", "chainID", "fgActivate"), _
							Array(objDb.rsElement(0), objDb.rsElement(1), objDb.rsElement(2), _
							objDb.rsElement(3), objDb.rsElement(4), objDb.rsElement(5), objDb.rsElement(6))
			...
			If strAncestor <> "" Then
				...
				blnErr = 0
				strQuery = "select a.projectID, b.ProjectName, a.SubTaskID, a.SubTaskName, ISNULL(a.taskID, 0) as ParentID, ISNULL(a.ChainID, '') as ChainID, b.fgActivate FROM " &_
...
If fgExecute and gMessage="" and isEmpty(session("arrBookmark")) then
	...
	Do Until objRoot.EOF
		k = k + 1
		arrRs(0).AddNew Array("pID", "pName", "sID", "sName", "sParentID", "chainID", "fgActivate"), _
						Array(objRoot(0), objRoot(1), objRoot(2), objRoot(3), objRoot(4), objRoot(5), objRoot(6))
		fgActive = objRoot(6)
		...
		if fgActive <> "0" then
			...
			strTmp = "<tr bgcolor='" & strColor & "'><td valign='top' class='black'>" & strTree & "</td></tr>"
		end if
...
var numpage = <%=session("NumPageAssigned")%>
	if (curpage < numpage) {
		document.frmAssigned.action = "individualass.asp?navi=NEXT"
...
var numpage = <%=session("NumPageAssigned")%>
	if (curpage > 1) {
		document.frmAssigned.action = "individualass.asp?navi=PREV"
		document.frmAssigned.submit();
	}
}
...
function go() {
	...
	if ((intpage > 0) && (intpage <= numpage) && (intpage != curpage)) {
		document.frmAssigned.action = "individualass.asp?Go=" + intpage
...
<form name="frmAssigned" method="post">
    		<%
			'--------------------------------------------------
  ...
  <tr> 
    <td>
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
		  ...
		  <tr>
			  <td class="blue" height="20" width="15%">&nbsp;&nbsp;<a href="../../Management/project/listofemp.asp" onMouseOver="self.status='Show the list of employees'; return true;" onMouseOut="self.status=''">Employees List</a></td>
			  <td class="blue-normal" width="85%">/ <%=personInfo(0)%>, <%=personInfo(1)%><%if UBound(personInfo)=2 then%>/<%=personInfo(2)%><%end if%></td>
		  </tr>
		  <tr align="center">
		    <td colspan="2" width="100%" class="title" height="50" align="center">Activities 
		      List </td>
		  </tr>
		</table>
    </td>
  ...
  <tr align="right" bgcolor="#99A89D"> 
    <td height="20" align="right" bgcolor="#E7EBF5">
		  ...
		  <tr> 
			...
			%>
	<input type="hidden" value="<%=personID%>" name="txthidden">
	<input type="hidden" value="<%=strFilename%>" name="txtpreviouspage">
</form>
</body>
</html>