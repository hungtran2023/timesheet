	...
	If Request.QueryString("act")<>"out" then
		...
		if cint(intType_)<>0 then
			...
			if Cint(intType_)=1 then
				strSearch = strSearch & " a.Outdated=1 AND b.UserID IN (SELECT StaffID FROM ATC_Employees WHERE fgIndirect = 0)"
			elseif Cint(intType_)=2 then
				strSearch = strSearch & " a.Outdated=1 AND b.UserID IN (SELECT StaffID FROM ATC_Employees WHERE fgIndirect = 1)"
            ...
            elseif Cint(intType_)=7 then
				strSearch = strSearch & "  a.Outdated=1 AND b.TypeOfPC= 7 "				
			...
			elseif Cint(intType_)=13 then		
				strSearch = strSearch & "  a.Outdated=1 AND b.UserID IN (SELECT TPUserID FROM ATC_TPUsers )"
