...
function Outbody(ByRef rsSrc,byval intPage, ByVal psize)
	...
	if not rsSrc.EOF then
		...
		For i = intStart to intFinish
		    ...
		    strLink = Showlabel(rsSrc("LicenceSoftware"))
            
            if fgUpdate then strLink="<a href='javascript:getdetail(" & rsSrc("LicenceRegisterID") & ");' " &_
			         "class='c' OnMouseOver = 'self.status=&quot;&quot; ; return true' OnMouseOut =" &_
			         "'self.status = &quot;&quot;'>" & Showlabel(rsSrc("LicenceSoftware")) & "</a>"
			         
			strOut = strOut & "<tr bgcolor=" & strColor & ">" &_
			         "<td valign='top' class='blue'>" & strLink & "</td>" &_
			         "<td valign='top' class='blue-normal'>" & Showlabel(rsSrc("TypeName")) & "</td>" &_
			         "<td valign='top' class='blue-normal'>" & Showlabel(rsSrc("NumberOfLicence")) & "</td>" &_
			         "<td valign='top' class='blue-normal' align='center'><input type='checkbox' name='chkRemove' value='" & rsSrc("LicenceRegisterID") & "'></td>" &_	
			         "</tr>" & chr(13)
			rsSrc.MoveNext
...
'--------------------------------------------------
	...
	if strAct="remove" then
		...
		if trim(arrSoftware)<>"" then
		    ...
		    strSql="SELECT Count(*) FROM ATC_LicenceInstallSoftwares WHERE LicenceRegisterID IN (" & arrSoftware & ")"
		    
		    Call GetRecordset(strSQL,rsData)
		    
		    
		    if cint(rsData(0)) =0 then
			    strSql="DELETE FROM ATC_LicenceSoftwares WHERE LicenceRegisterID IN (" & arrSoftware & ")"
                strError= ExecuteSQL(strSql)
                gMessage="Selected item have been removed."
                if strError<>"" then gMessage=strError
            else 
                gMessage="Please remove software installations assigned with package first."
            end if
	...
	strSQL="SELECT a.*, b.TypeName FROM dbo.ATC_LicenceSoftwares a " & _
                    "LEFT JOIN ATC_LicenceTypes b ON a.TypeOfLicence=LicenceTypeID "

	if trim(strSearch<>"") then
		intSearchType=Request.Form("lstType")
		if CInt(intSearchType)=1 then 
			strSearch=" LicenceSoftware like '%" & trim(strSearch) & "%'"
		elseif CInt(intSearchType)=2 then
			strSearch="  b.TypeName like '%" & trim(strSearch) & "%'"
		end if
		strSQL=strSQL & " WHERE " & strSearch 
	end if
	
	strSQL=strSQL & " ORDER BY LicenceSoftware"
...
function search() {
	var tmp = document.navi.txtsearch.value;
		tmp = escape(tmp);
	document.navi.action = "SoftwareRegister.asp?search=" + tmp;
...
function getdetail(varid){
	...
	document.navi.action = "RegisterDetail.asp?act=EDIT";
	document.navi.target = "_self";
	document.navi.submit();
}
...
function addnew(){
	...
	document.navi.action = "RegisterDetail.asp?act=ADD";
	document.navi.target = "_self";
	document.navi.submit();
}
...
function RemoveSoftware()
{
	...
	if (agree)
	{
		window.document.navi.action = "SoftwareRegister.asp?act=remove"	;		
...
var numpage = <%=intPageCount%>
	if (curpage < numpage) {
		...
		curpage=<%=intPage+1%>
		document.navi.action = "SoftwareRegister.asp?navi=" + curpage;
		document.navi.target = "_self";
		document.navi.submit();
	}
}
...
var numpage = <%=intPageCount%>
	...
	if ((intpage > 0) && (intpage <= numpage) && (intpage != curpage)) {
		document.navi.action = "SoftwareRegister.asp?navi=" + intpage;
		document.navi.target = "_self";
		document.navi.submit();		
	}
}
...
<%end if%>
                  ...
                  <td align="right" width="20%" valign="middle"> 
                    <select name='lstType' size='1' height='26px' width='70px' style='width:95%;height=24px;' class='blue-normal'>
						<option value='1'>Software Licences</option>
						<option value='2'>Licence Type</option>
					</select>
                ...
                <tr> 
                  <td class="title" height="50" align="center" colspan="5">Software Licences</td>
                </tr>
              </table>
            </td>
          </tr>
                      ...
                      <tr> 
                        <td bgcolor="#617DC0"> 
                          <table width="100%" border="0" cellspacing="1" cellpadding="5">
                            <tr bgcolor="8CA0D1"> 
                              <td class="blue" align="center"  width="35%">Register name</td>
                              <td class="blue" align="center" width="27%">Type of Licence</td>
                              <td class="blue" align="center" width="28%">Number of Licence</td>
