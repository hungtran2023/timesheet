...
'--------------------------------------------------
	...
	intStaffID  = Request.Form("txtUserid")
	
	strSql="SELECT Fullname,Username FROM HR_TPStaff WHERE TPUserid=" & intStaffID
	...
	If Request.QueryString("act") = "vmya" Then
		strError	=  tmsInitialForTP(intStaffID,intMonth,intYear)
...
{
	if (ns)
		document.location = "timesheetTP.asp";
	else
	{
		window.document.frmtms.action = "timesheetTP.asp";
		window.document.frmtms.target = "_self";
		window.document.frmtms.submit();
	}	
}
...
<input type="hidden" name="txtUserid" value="<%=intStaffID%>">
