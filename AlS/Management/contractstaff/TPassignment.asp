...
if proID <>"" then 'draw sub task
	...
	strQuery="SELECT f.UserName as FullName,d.LevelName as Jobtitle,a.* " & _
			"FROM ATC_Assignments a " & _
				"INNER JOIN ATC_PersonalInfo b ON a.StaffID=b.PersonID " & _
				"INNER JOIN ATC_TPUsers c ON b.PersonID=c.TPUserID " & _
				"INNER JOIN ATC_Users f ON b.PersonID=f.UserID " & _
				"INNER JOIN ATC_TPLevel d ON c.LevelID=d.LevelID " &_
			"WHERE a.fgDelete=0 AND b.fgDelete=0 AND subTaskID IN (SELECT subTaskID FROM ATC_Tasks WHERE projectID='" & proID & "') ORDER BY FullName"
...
function assignment(taskid) { //v2.0
    ...
    document.frmassign.action = "selectTPStaff.asp?outside=1&taskid=" + taskid
...
function remove() {
  if (chkremove()==true) {
  	document.frmassign.action = "TPassignment.asp?act=REMOVE"
	document.frmassign.target = "_self"
	document.frmassign.submit();
  }
}
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0">
        ...
        <tr>
          <td class="blue" height="20" align="left">&nbsp;&nbsp;
			<a href="../../Management/contractstaff/TPStaffAssignment.asp?act=BACK" onMouseOver="self.status='Show the list of projects'; return true;" onMouseOut="self.status=''">Project List</a> 
			&nbsp;</td>
        </tr>
        <tr valign="middle"> 
