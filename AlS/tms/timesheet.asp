...
<%
	...
	Dim strFirstDay, strParm, strURLSetHour, strColorOpt, strError, strShow, varTimesheet, varEvent,dLeaveDate,arrFingerprint
	Dim strDateLock
...
'--------------------------------------------------
    ...
    if session("UserType")=3 then
        Response.Redirect("timesheetTP.asp")
    end if	
...
'response.write (intUserID)	
	...
	dLeaveDate=LeaveDateUser(intUserID)		
	
	if not isnull(dLeaveDate) then
		if dLeaveDate<date() then	
			if month(dLeaveDate)=intMonth and year(dLeaveDate)=intYear then
				intDayCount=day(dLeaveDate)	
			else
				intDayCount=0
			end if
		end if
	end if
'Response.Write 	intDayCount & "<br>"
'--------------------------------------------------
...
'--------------------------------------------------
	...
	If (IsEmpty(session("varTimesheet")) And IsEmpty(session("varEvent"))) Or (Request.QueryString("act") = "vmya") Then
		...
		strError	=  tmsInitial(intUserID,intMonth,intYear)
...
'--------------------------------------------------	
	strSQL="SELECT DATENAME(dw , CalanderDate) ,CalanderDate,   CONVERT(VARCHAR(5),Enter,108) as Enter, CONVERT(VARCHAR(5),ExitHour,108) as ExitHour, CAST(FORMAT(Duration/60,'00') as nvarchar) +':' +CAST(FORMAT(Duration%60,'00') as nvarchar) as Duration  , " & _
			...
			"WHERE (MONTH(Tdate) = " & intMonth & ") AND (YEAR(Tdate) =" & intYear & ") AND PersonID=" & intUserID & ") b ON a.CalanderDate=b.Tdate " &_
...
{ //v2.0
	...
	strFeatures = "top=5,left="+(screen.width/2-230)+",width=550,height=600,toolbar=no," 
...
function sethour(row, col, kind)
{
	...
	objSetHourWindow = window.open('tms_writehour.asp?r=' + row + '&c=' + col + '&k=' + kind + '&m=' + window.document.frmtms.lbmonth.options[window.document.frmtms.lbmonth.selectedIndex].value + '&y=' + window.document.frmtms.lbyear.options[window.document.frmtms.lbyear.selectedIndex].value, "MyNewWindow2", strFeatures);
...
%>					  	
              <tr> 
                <td height="20" bgcolor="#FFC6C6"><img src="../images/cross.gif" width="8" height="20"></td>
<%
...
'if intUserID=888 then Response.Write kk & ":" & intDayCount & "-" & (kk < intDayCount)  & "<br>"
						...
						ElseIf (kk = intDayCount And CInt(intMonth) = Month(Date) And CInt(intYear) = Year(Date)) Then
							...
							If dblHour > 0 Then
								if not rsAssignment.EOF OR rsAssignment.Recordcount>0 then
...
<%				End If
			Else%>                      
              <tr> 
                <td width="1%" bgcolor="#FFC6C6" class="white" height="20">&nbsp;</td>
...
<%	If CInt(intMonth) = Month(Date) And CInt(intYear) = Year(Date) Then%>			  
                <td  bgcolor="#FFC6C6" class="white"><img src="../images/cross.gif" width="8" height="20"></td>
...
<%	Else%>                
                <td  bgcolor="#FFC6C6" class="white" height="20">&nbsp;</td>
...
%>
                <td colspan="2" bgcolor="#FFE1E1" class="blue" height="20"><b>&nbsp;<%=varEvent(0,0,ii)%></b></td>
<%
		Else
%>          
                <td colspan="2" class="blue-normal" bgcolor="#FFE1E1" height="20">&nbsp;<%=varEvent(0,0,ii)%></td>
<%
		End If
