...
'--------------------------------------------------
	...
	intOldAssign= Request.QueryString("assign")
	
'	Response.Write intMonth & " " & intYear
'	Response.End
...
If strType = "1" Then
	...
	If strAction = "U" Then
		intOldAssignmentID = intOldAssign
		...
		if intUserType=3 then
            strError = tmsUpdateSubtaskforTP(intOldAssignmentID, intNewAssignmentID, intMonth, intYear, intUserID)
        else
		    strError = tmsUpdateSubtask(intOldAssignmentID, intNewAssignmentID, intMonth, intYear, intUserID)
		end if
...
<script LANGUAGE="javascript">
<!--
    opener.document.frmtms.action = "timesheetTP.asp?act=vmya";
...
<%          end if		
		Else
			Response.Write "<table width='450' border='0' cellspacing='0' cellpadding='0'><tr bgcolor='#E7EBF5'><td class='red'>&nbsp;<b>" & strError & "</b></td></tr></table>"
		End If
	Else
		...
		For ii = 1 To Request.Form("chkget").Count
			...
			if intUserType=3 then
			    Call tmsAddsubtaskforTP(strPID, strPName, intParentID, strSubTask, intAssignmentID, intMonth, intYear)
			else
...
If objDb.dbConnect(strConnect) then
	If Request.QueryString("search") = "" Then
			...
			strQuery = "SELECT b.projectID, c.ProjectName, b.SubTaskID, b.SubTaskName, ISNULL(b.taskID, 0), ISNULL(b.ChainID, ''), 0 AS fgLeaf, a.AssignmentID FROM " &_
						"((SELECT * FROM ATC_Assignments WHERE StaffID = " & intUserID & " AND fgDelete=0) a " &_
...
function p_search()
{
	...
	{	
		url = "tms_addsubtask.asp?search=Yes&m=" + "<%=intMonth%>" + "&y=" + "<%=intYear%>" + "&act=" + "<%=strAction%>" + "&assign=" + "<%=intOldAssign%>" + "&sel=" + "<%=strType%>";
...
function submitform()
{
	...
	if (fgexist == 1)				
	{
		url = "tms_addsubtask.asp?sel=1&m=" + "<%=intMonth%>" + "&y=" + "<%=intYear%>" + "&act=" + "<%=strAction%>" + "&assign=" + "<%=intOldAssign%>";
...
function showall()
{
	url = "tms_addsubtask.asp?sel=" + "<%=strType%>" + "&m=" + "<%=intMonth%>" + "&y=" + "<%=intYear%>" + "&act=" + "<%=strAction%>" + "&assign=" + "<%=intOldAssign%>";
              ...
              <table width="150" border="0" cellspacing="5" cellpadding="0" height="20" name="aa">
                <tr> 
                  <td width="75" bgcolor="#8CA0D1" onMouseOver="this.style.backgroundColor='#7791D1';" onMouseOut="this.style.backgroundColor='#8CA0D1';" height="20"> 
                    <div align="center" class="blue"><a href="javascript:p_search();" class="b">Search</a></div>
                  </td>
