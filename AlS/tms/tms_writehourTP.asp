...
'--------------------------------------------------
	...
	If Request.QueryString("act") = "U" Then
		...
		If strError1 = "" Then
			...
			Else
				 strError = tmsWriteHourforTP(intUserID, Col, Row, strType, intMonth, intYear, "txtoffhour","txtOTNormal","txtOTNight", "txtnote")
				 if strError="" then
...
function writehour()
{
	...
	{
		if (ns)
			document.location = "tms_writehourTP.asp?act=U&r=" + row + "&c=" + col + "&k=" + kind + "&m=" + "<%=intMonth%>" + "&y=" + "<%=intYear%>";
		else
		{
			window.document.frmtms.action = "tms_writehourTP.asp?act=U&r=" + row + "&c=" + col + "&k=" + kind + "&m=" + "<%=intMonth%>" + "&y=" + "<%=intYear%>";
