	...
	If Request.QueryString("act") = "U" Then
		...
		If strType = "E" Then 
			If trim(varEvent(0, 0, Row)) = "Annual Holiday" Then
				...
				'If myCmd("CurLeave") = myCmd("LeaveDue") Then
				intCurRequest=0
				if Request.Form("txtoffhour")<>"" then 
					intCurRequest = CDbl(Request.Form("txtoffhour"))
					if intBalance<0 or intBalance + dblOldValue-intCurRequest<0 then
						strError1 = "This application exceeds your annual leave entitlement.<br>Therefore you could not get access to it."
					End	If
				end if
				...
				If strError1 = "" Then
					If Request.Form("txtoffhour") <> "" Then
						If CDbl(Request.Form("txtoffhour")) <> 4.5 And CDbl(Request.Form("txtoffhour")) <> 4 And CCur(Request.Form("txtoffhour")) <> CCur(intWorkHours) Then
		...
		If strError1 = "" Then
			If (CDate(strDay) <> Date() And ((strType="E" And CInt(Row) <> 2) Or (strType="P"))) Or (CDate(strDay) < Date() And strType="E" And CInt(Row) = 2) Then 
				strError1 = "You should visit to Admin department for help."
			Else
				 strError = tmsWriteHour(intUserID, Col, Row, strType, intMonth, intYear, "txtoffhour","txtOTNormal","txtOTNight", "txtnote")
				 if strError="" then
					
						if Request.Form("chkPersonalDev")<>"" then					
							strError = tmsWriteHourForStaffDevelopment(intUserID, Col, 0, "E", intMonth, intYear, 0.5)
...
<%							
			...
			If strError = "" Then
				varCookies = "timesheet_" & trim(session("USERID"))
...
<script LANGUAGE="javascript">
<!--
	opener.document.frmtms.action = "timesheet.asp?act=vpa"
...
function writehour()
{
	...
	{
		if (ns)
			document.location = "tms_writehour.asp?act=U&r=" + row + "&c=" + col + "&k=" + kind + "&m=" + "<%=intMonth%>" + "&y=" + "<%=intYear%>";
		else
		{
			window.document.frmtms.action = "tms_writehour.asp?act=U&r=" + row + "&c=" + col + "&k=" + kind + "&m=" + "<%=intMonth%>" + "&y=" + "<%=intYear%>";
...
function window_onload() 
{
	if ("<%=fgDayOff%>" == 0)
		document.frmtms.txtoffhour.focus();
	//else
	//	document.frmtms.txtoverhour.focus();
}
...
<%		If (strType = "P") Or (strType = "E" And (Row = 0 Or Row = 1)) Then%>                
		  <!--<tr bgcolor="#C0CAE6"> 
            <td width="99" class="blue-normal" align="right" height="26">Overtime Hours&nbsp; </td>
            <td width="151" class="text-blue01"> -->
...
<%		End If%>          
          <tr bgcolor="#C0CAE6"> 
            ...
            <td width="151" class="text-blue01"> 
              <input type="text" name="txtnote" class="blue-normal" size="10" style="width:120" tabindex="3" value="<%=strNote%>">
            </td>
          </tr>
