...
'--------------------------------------------------
	...
	strTPUserid  = Request.Form("txthidden")

'Response.Write 	strTPUserid
...
'--------------------------------------------------
	...
	If (IsEmpty(session("varTimesheet")) And IsEmpty(session("varEvent"))) Or (Request.QueryString("act") = "vmya") Then
		'strError	=  tmsInitial(strTPUserid,intMonth,intYear)
		strError	=  tmsInitialForTP(strTPUserid,intMonth,intYear)
		
'Response.Write "<br>" &	strTPUserid & "--" & intMonth & "--" & intYear
...
'--------------------------------------------------
	...
	strFunction = "<a class='c' href='javascript:back_menu();' onMouseOver='self.status=&quot;Return to main menu page&quot;;return true' onMouseOut='self.status=&quot;&quot;;return true'>Main Menu</a>&nbsp;&nbsp;&nbsp;<img height='5' src='../../images/dot.gif' width='5'>&nbsp;&nbsp;&nbsp;" & _
				  "<a class='c' href='javascript:selstaff();' onMouseOver='self.status=&quot;Select employee to view timesheet&quot;;return true' onMouseOut='self.status=&quot;&quot;;return true'>Select Contract Staffs</a>&nbsp;&nbsp;&nbsp;<img height='5' src='../../images/dot.gif' width='5'>&nbsp;&nbsp;&nbsp;" & _
    ...
    strSQL="SELECT Fullname, department FROM HR_TPStaff WHERE TPUserID=" & strTPUserid
	Call GetRecordset(strSQL ,rsTpStaff) 
	
	strTitle1	= "Timesheet of <b>" & rsTpStaff("Fullname") & " - " & rsTpStaff("department") & "</b>"
...
function viewtms()
{
	...
	URL = "rpt_TPstaff_tms.asp?act=vmya";
...
function selstaff()
{
	...
	{
		objStaffWindow = window.open('rpt_select_staff.asp?type=3&view=t', "MyNewWindow", strFeatures);
	}
...
function printpage()
{
	...
	objPrintWindow = window.open('rpt_print_preview.asp?type=3&m=' + window.document.frmtms.lbmonth.options[window.document.frmtms.lbmonth.selectedIndex].value + '&y=' + window.document.frmtms.lbyear.options[window.document.frmtms.lbyear.selectedIndex].value + '&s=' + '<%=strTPUserid%>', "MyNewWindow", strFeatures);
...
%>                        
                <td width="118" class="blue" bgcolor="#FFF2F2"><a href="javascript:void(0);" title="<%=varTimesheet(intDayCol-3,0,ii)%>" onMouseOver="self.status=&quot;<%=varTimesheet(intDayCol-3,0,ii)%>&quot;;return true" onMouseOut="self.status='';return true" class="c"><b>&nbsp;-<%=showlabel(varTimesheet(intDayCol-3,0,ii))%></b></a></td>
<%
...
<input type="hidden" name="txthidden" value="<%=strTPUserid%>">
