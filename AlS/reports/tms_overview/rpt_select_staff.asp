...
'--------------------------------------------------
	...
	intUserStyle=Request.Querystring("type")
...
function sort(kind)
{
	document.frmreport.action = "rpt_select_staff.asp?act=vps" + kind + "&view=" + "<%=strView%>";
...
function viewpage(kind)
{
	...
	if (kind == 1)
	{
		...
		{
			document.frmreport.action = "rpt_select_staff.asp?act=vpa" + kind + "&view=" + "<%=strView%>"+ "&type=" + "<%=intUserStyle %>";
			document.frmreport.submit();
		}	
	}
	else
...
function search(kind)
{
	if (kind == "1")
	{
		...
		{
			document.frmreport.action = "rpt_select_staff.asp?act=vra" + kind + "&view=" + "<%=strView%>" + "&type=" + "<%=intUserStyle %>";
			document.frmreport.submit();	
		}
	}	
	else
...
{
var type = "<%=intUserStyle%>";
	
	if (type=="1") 
	    opener.document.frmtms.action = "rpt_staff_tms.asp";
	else
	    opener.document.frmtms.action = "rpt_TPstaff_tms.asp";
