...
case "SAVE"
	...
	if not isEmpty(session("rsItem")) then
		...
		rsItem.Find "ProjectType like " & tmp1,, adSearchForward
	...
	if gMessage="" then
		...
		if ret then
			...
			strQuery = "UPDATE ATC_ProjectTypes SET ProjectType = '" & strNameItem & "' WHERE ProjectTypeID = " & rsItem("ProjectTypeID")
...
case "ADD"
	...
	if gMessage = "" then
		...
		if ret then
			strNameItem = replace(strNameItem, "'", "''")
			strQuery = "INSERT INTO ATC_ProjectTypes(ProjectType) VALUES('" & strNameItem & "')"
			if not objDB.runActionquery(strQuery) then
		      gMessage = objDb.strMessage
		    else
	...
	if countU>0 then
	  ...
	  if ret then
	    ...
	    for ii = 1 to countU
			...
			strQuery = "DELETE ATC_ProjectTypes WHERE ProjectTypeID = " & varID
			if not objDB.runActionquery(strQuery) then
				rsItem.Bookmark = int(varBookmark)
				strDonot = strDonot & " " & rsItem("ProjectType") & ","
			end if
	    next
	    if strDonot<>"" then
...
If session("READYIN")<> True Then
	...
	strQuery = "select * FROM ATC_ProjectTypes ORDER BY ProjectType"
...
if stract = "EDIT" and strNameItem="" then
	varID = Request.Form("txthidden")
	rsItem.Bookmark = int(varID)
	strNameItem = rsItem("ProjectType")
end if
...
function _edit(varid){
	...
	document.frminput.action = "typeofproject.asp?act=EDIT"
...
function chkremove() {
 ...
 if (fg == false) alert("No type of project selected.")
 return(fg)
}

function _cancel(){
  	document.frminput.action = "typeofproject.asp?act=CANCEL"
	document.frminput.target = "_self";
	document.frminput.submit();
}
...
var varid = "<%=varID%>";
	document.frminput.txthidden.value = varid;
  	document.frminput.action = "typeofproject.asp?act=SAVE"
	document.frminput.target = "_self";
	document.frminput.submit();
}
...
function _add(){
	...
	if(document.frminput.txtname.value=="") {
		alert("Please enter a value");
  		...
  		document.frminput.action = "typeofproject.asp?act=ADD"
		document.frminput.target = "_self";
		document.frminput.submit();
	}
}
...
function _remove() {
  if (chkremove()==true) {
  	document.frminput.action = "typeofproject.asp?act=DELETE"
	document.frminput.target = "_self";
	document.frminput.submit();
  }
}
...
var numpage = <%=session("NumPagein")%>
	if (curpage < numpage) {
		document.frminput.action = "typeofproject.asp?navi=NEXT"
		document.frminput.target = "_self"
		document.frminput.submit();
	}
}
...
var numpage = <%=session("NumPagein")%>
	if (curpage > 1) {
		document.frminput.action = "typeofproject.asp?navi=PREV";
		document.frminput.target = "_self"
		document.frminput.submit();
	}
}
...
function go() {
	...
	if ((intpage > 0) && (intpage <= numpage) && (intpage != curpage)) {
		document.frminput.action = "typeofproject.asp?Go=" + intpage;
		document.frminput.target = "_self";
		document.frminput.submit();		
	}
}
    ...
    <tr> 
      <td align="center"> 
        ...
        <tr bgcolor="<%if gMessage="" then%>#FFFFFF<%else%>#E7EBF5<%end if%>">
          ...
          <tr align="center"> 
            <td class="title" height="50" align="center" colspan="3">Type of Project</td>
          </tr>
          <tr align="center"> 
            <td class="blue-normal" height="30" align="left" width="80"> 
              &nbsp;&nbsp;Project Type&nbsp; </td>
            <td class="blue" height="30" align="left" width="170">
              <input type="text" name="txtname" class="blue-normal" size="20" style=" width:165" maxlength="50" value="<%=strNameItem%>">
            </td>
            <td class="blue-normal" height="30" align="left" width="360"> 
<%if stract <> "EDIT" then%>
                  ...
                  <td bgcolor="#617DC0"> 
                    <table width="100%" border="0" cellspacing="1" cellpadding="5">
                      <tr bgcolor="8CA0D1" align="left"> 
                        ...
                        <td class="blue" width="80%" align="center">Project Type</td>
