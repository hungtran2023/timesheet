...
case "SAVE"
	...
	if not isEmpty(session("rsItem")) then
		...
		rsItem.Find "JobTitle like " & tmp1,, adSearchForward
	...
	if gMessage = "" then
		...
		if ret then
			...
			strQuery = "UPDATE ATC_JobTitle SET JobTitle = '" & strNameItem & "' WHERE JobTitleID = " & rsItem("JobTitleID")
...
case "ADD"
	...
	if gMessage = "" then
		...
		if ret then
			strNameItem = replace(strNameItem, "'", "''")
			strQuery = "INSERT INTO ATC_JobTitle(JobTitle) VALUES('" & strNameItem & "')"
			if not objDB.runActionquery(strQuery) then
		      gMessage = objDb.strMessage
		    else
	...
	if countU>0 then
	  ...
	  if ret then
	    ...
	    for ii = 1 to countU
			...
			strQuery = "DELETE ATC_JobTitle WHERE JobTitleID = " & varID
			if not objDB.runActionquery(strQuery) then
				rsItem.Bookmark = int(varBookmark)
				strDonot = strDonot & " " & rsItem("JobTitle") & ","
			end if
	    next
	    if strDonot<>"" then
...
If session("READYIN")<> True Then
	...
	strQuery = "select * FROM ATC_JobTitle ORDER BY JobTitle"
...
function _edit(varid){
	...
	document.frminput.action = "jobtitle.asp?act=EDIT"
...
function _cancel(){
  	document.frminput.action = "jobtitle.asp?act=CANCEL"
	document.frminput.target = "_self";
	document.frminput.submit();
}
...
var varid = "<%=varID%>";
	document.frminput.txthidden.value = varid;
  	document.frminput.action = "jobtitle.asp?act=SAVE"
	document.frminput.target = "_self";
	document.frminput.submit();
}
...
function _add(){
	...
	if(document.frminput.txtname.value=="") {
		alert("Please enter a value");
  		...
  		document.frminput.action = "jobtitle.asp?act=ADD"
		document.frminput.target = "_self";
		document.frminput.submit();
	}
}
...
function _remove() {
  if (chkremove()==true) {
  	document.frminput.action = "jobtitle.asp?act=DELETE"
	document.frminput.target = "_self";
	document.frminput.submit();
  }
}
...
var numpage = <%=session("NumPagein")%>
	if (curpage < numpage) {
		document.frminput.action = "jobtitle.asp?navi=NEXT"
		document.frminput.target = "_self"
		document.frminput.submit();
	}
}
...
var numpage = <%=session("NumPagein")%>
	if (curpage > 1) {
		document.frminput.action = "jobtitle.asp?navi=PREV";
		document.frminput.target = "_self"
		document.frminput.submit();
	}
}
...
function go() {
	...
	if ((intpage > 0) && (intpage <= numpage) && (intpage != curpage)) {
		document.frminput.action = "jobtitle.asp?Go=" + intpage;
		document.frminput.target = "_self";
		document.frminput.submit();		
	}
}
    ...
    <tr> 
      <td align="center"> 
        ...
        <tr bgcolor="<%if gMessage="" then%>#FFFFFF<%else%>#E7EBF5<%end if%>">
          ...
          <tr align="center"> 
            <td class="title" height="50" align="center" colspan="3">Job Title</td>
          </tr>
          <tr align="center"> 
            <td class="blue-normal" height="30" width="46">
              &nbsp;&nbsp;Job title&nbsp; </td>
            <td class="blue" height="30" align="left" width="177"> 
              <input type="text" name="txtname" class="blue-normal" size="20" style=" width:165" maxlength="50" value="<%=strNameItem%>">
            </td>
            <td class="blue-normal" height="30" align="left" width="384"> 
<%if stract <> "EDIT" then%>
                  ...
                  <td bgcolor="#617DC0"> 
                    <table width="100%" border="0" cellspacing="1" cellpadding="5">
                      <tr bgcolor="8CA0D1" align="left"> 
                        ...
                        <td class="blue" width="80%" align="center">Job title</td>
