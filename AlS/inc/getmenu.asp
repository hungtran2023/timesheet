...
function selectday(ByVal vname, ByVal vselected, ByVal fgNull)
	strlst = "<select name='" & vname & "' size='1' height='26px' width='40px' " &_
			"style='width:40px;height=24px;' " &_
			"class='blue-normal'"
...
function selectmonth(ByVal vname, Byval vselected, ByVal fgNull)
	...
	strlst = "<select name='" & vname & "' size='1' height='26px' width='50px' " &_
			"style='width:50px;height=24px;'" &_
			"class='blue-normal'"
	if fgNull<>-1 then strlst=strlst & " onClick='CheckMode(this)'"
	strlst=strlst & ">"
	strTmp = ""
...
'------append default menu (tools)
  ...
  ReDim Preserve arrMenu(4, intCnt + 4)
  intCnt = intCnt + 1
  arrMenu(0, intCnt) = 1 ' flag
  ...
  intCnt = intCnt + 1
  arrMenu(0, intCnt) = 0 ' flag
  arrMenu(1, intCnt) = "Forms" 'objRs2("Description")
  arrMenu(2, intCnt) = "tools/listforms.asp" 'objRs2("Form")
  arrMenu(3, intCnt) = 0 'objRs2("varLevel")
  arrMenu(4, intCnt) = 3
...
intDailyTimesheet=session("GroupManager")
	...
	if strChose = "aisnet/Employee/AtlasStaff" then
		strLink1 = "<font color='#CA0000'>" & "Atlas Staff" & "</font> <img src='images/new.gif'/>"
	else
		strLink1 = "Atlas Staff"
	end if
...
strResult = strResult & "<a target='_blank' rel='noopener noreferrer' href='" & session("strHTTP") & "aisnet/Employee/AtlasStaff' OnMouseOver = 'self.status=&quot;Fill In Timesheet&quot; ;"
strResult = strResult & " return true' OnMouseOut = 'self.status = &quot;&quot;'>" & strLink1 & "</a> <img src='images/new.gif'/></td></tr>"
...
End if
	if isArray(arrMenu) then  ' Special rights
		...
		if Mid(strChoseMenu, 1, 1) = "A" then 'because Financial belong to Management Console ; strChoseMenu = "AA"
			...
			strRet = MakepartLevel2(arrMenu, strValue, strChose, "Timesheets", strChoseMenu, "AA")
			if strRet <> "" then
				strResult = strResult & strRet
			end if
			'Show the items belongs to Employee
			strRet = MakepartLevel2(arrMenu, strValue, strChose, "Employees", strChoseMenu, "AB")
			if strRet <> "" then
				strResult = strResult & strRet
			end if
			'Show the items belongs to Project
			strRet = MakepartLevel2(arrMenu, strValue, strChose, "Projects", strChoseMenu, "AC")
			if strRet <> "" then
				strResult = strResult & strRet
			end if

			'Show the items belongs to Financial
			strRet = MakepartLevel2(arrMenu, strValue, strChose, "Financial", strChoseMenu, "AD")
			if strRet <> "" then
				strResult = strResult & strRet
			end if
			...
			'Show the items belongs to Contract Staff
			strRet = MakepartLevel2(arrMenu, strValue, strChose, "Contract Staff", strChoseMenu, "AG")
			if strRet <> "" then
				strResult = strResult & strRet
			end if
			
			'Show the items belongs to Training
			strRet = MakepartLevel2(arrMenu, strValue, strChose, "Atlas Training", strChoseMenu, "AH")
			if strRet <> "" then
				strResult = strResult & strRet
			end if
			
			'Show the items belongs to Offshore Staff
			strRet = MakepartLevel2(arrMenu, strValue, strChose, "Offshore Staffs", strChoseMenu, "AI")
			if strRet <> "" then
				strResult = strResult & strRet
			end if
		end if
